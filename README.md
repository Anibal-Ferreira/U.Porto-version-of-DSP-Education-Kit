...
## LAB 12
**Lab Summary:** Laboratory experimentation on adaptive filtering in a system identification configuration exploiting the steepest descent concept, the impact of the adaptation factor (β), and the importance of the bandwidth of the excitation signal.

![Lab 12](LAB_12/LAB13_sm.png)

This lab is an adapted version of the original ARM “LAB 6 Adaptive Filters” that is included in the original ARM Module_13/Lab/ folder. Originally, section 6 of this lab included an IIR system identification experiment where the result of the adaptive filtering was checked in the frequency domain and required off-line data analysis. In the modified version, the system identification experiment involves an FIR filter whose (reference) impulse response is defined in the main() section of the code. The advantage of this is that the STM32F7 LCD screen is now used to display in "real-time" both the reference impulse response (using BLUE color) and the adaptive filter impulse response (using the RED color). This facilitates real-time visualization of the convergence process in four modes: successful and slow (when the adaptation step size is very small), successful and fast (when the adaptation step size is higher), unsuccessful (when the adaptation step size is too large and the adaptive filter coefficients vary in random way), and unable to converge (when the excitation signal is a sinusoid instead of noise). 
## Protective casing
